...
<div class="row-fluid sortable">
  <div class="box span12">
    <div class="box-header well" data-original-title>
      <h2><i class="icon-user"></i> All Race </h2>
      <div class="box-icon"></div>
    </div>
    <div class="box-content">
		
  <div class="page-header" style="padding-bottom:5px;margin:8px 0;"><h3>Search</h3></div>
    <form name="frm_search" id="frm_search" method="post" action="index.php?model=race">
    	<input type="hidden" name="action" id="action" value="" />
		<table  style="border:none;" cellpadding="4" cellspacing="2">
            <tr>
                <td align="right">Race Type : <input type="search" name="sr_race_type" id="sr_race_type" value="<?=$_REQUEST["sr_race_type"]?>"  /></td>
                <td align="right"></td>
      ...
      <div class="control-group" style="text-align:right;margin:20px 0 0 0;">
        <div class="controls"> 
           <a class="btn btn-success" href="index.php?model=race&action=add"><i class="icon-plus icon-white"></i> Add New Race </a> 
        </div>
      </div>
      <br />
      <table class="table table-bordered table-striped table-condensed datatable1">
        ...
        <?php 
			...
			{
				$view   = "index.php?model=race&action=view&id=".$datum->id;
				$edit   = "index.php?model=race&action=edit&id=".$datum->id;
				$delete = "index.php?model=race&action=delete&id=".$datum->id;
	
				$timeStamp=$objComm->dateDucationFormat1($datum->race_start_datetime,$datum->race_end_datetime);
			?>
				<tr id="<?=$datum->id?>">
				...
				<td class="center"><?=date("Y-m-d",strtotime($datum->race_date))?></td>
				<td class="center"><span class="label"><?=$datum->race_active?></span></td>
				...
				<a class="btn btn-danger delete_datum" id="deleted_<?=$datum->id?>" data="race;id;<?=$datum->id?>" href="#" alt="Delete Athletes" title="Delete Athletes"> <i class="icon-trash icon-white"></i></a></td>
				</td>
				</tr>
		<?php } ?>
        </tbody>
