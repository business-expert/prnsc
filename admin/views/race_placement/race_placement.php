...
<style>
  ...
  #srtoTabs ul li a {border-left: 1px solid;
	...
	.fb_image_size { width:50px;height:50px; }
</style>
...
<?php  foreach($data as $datum) { ?>	
	...
	else
	{
		...
		#Condition to categorize athletic type to participate to the race.
		if($type=="All") { $CONDITON=""; } else { $CONDITON=" WHERE athlete.type='".$type."'"; }
		$SQL="SELECT athlete.id AS athelete_id,athlete.name,athlete.facebook_url,athlete.photo_url FROM athlete".$CONDITON;
		$type="";
	...
	?>
	  ...
	  <!-- START Athletics Listing -->
		  <ul id="sortable1" class="droptrue rt<?=$datum->id?> orderSort_rt<?=$datum->id?>">
		  <?php $counter=1; foreach($NEWDATA as $NEWDATUM) { ?>
		  <!-- Code to check Whether give Photo-URL is exist or Not. -->
		  <?php $image = (exif_imagetype($NEWDATUM->photo_url) == IMAGETYPE_JPEG) ? $NEWDATUM->photo_url : DEFAULT_ATHLETES_PROFILE_AVTAR; ?>			 
			  ...
			  <a href="<?=$NEWDATUM->facebook_url?>" data="<?=$datum->id?>#<?=$NEWDATUM->athelete_id?>#<?=$NEWDATUM->position?>#<?=$NEWDATUM->id?>">
				...
				<span class="positionCounter"><?=$counter?></span>
				 <span class="userImg">
					<img src="<?=$image?>" class="fb_image_size" alt="">
				  </span>
				  &nbsp;&nbsp;
				  <span class="userName">
					<?=$NEWDATUM->name?>
	  ...
	  <a href="#" id="add<?=$datum->id?>" data="ADD" rel="rt<?=$datum->id?>" style="margin-left:10px;" class="saveRacePlacement <?=($action=="ADD" && $counter>1) ? "show" : "hide";?>">
	  ...
	  <a href="#" id="update<?=$datum->id?>" data="UPDATE" rel="rt<?=$datum->id?>" style="margin-left:10px;" class="updateRacePlacement <?php echo ($action=="UPDATE" && $counter>1) ? "show" : "hide"; ?>">
		<button type="button" class="btn btn-primary">
			UPDATE
		</button>
	  </a>
	  
	   <a href="#" id="delete<?=$datum->id?>" data="DELETE" rel="rt<?=$datum->id?>" style="margin-left:10px;" class="deleteRacePlacement <?php echo ($action=="UPDATE" && $counter>1) ? "show" : "hide"; ?>">
		<button type="button" class="btn btn-primary">
			RESET
		</button>
	  </a>
	  <a href="#" id="publish<?=$datum->id?>" data="PUBLISH" rel="rt<?=$datum->id?>" style="margin-left:10px;" class="publishRacePlacement <?=($counter>1 && $datum->race_flag==0 && $action=="UPDATE") ? "show" : "hide";?>">
		<button type="button" class="btn btn-primary">
			PUBLISH
		</button>
	  </a>
  ...
  $(function() {
    $( "ul.droptrue" ).sortable({
	  ...
	  update: function(ev,ui){
			$("#srtoTabsContent").children().each(function(){
				if($(this).css("display")=="block")
				{
					var counter=1;
					$(".orderSort_"+$(this).attr("id")).children().each(function() {	
						$(this).find("a span.positionCounter").html(counter);
						counter = counter + 1;
					});
				}			
			});
			//$("ul.<?=$datum->id?>").children().each(function() {		
...
<script>
	$(document).ready(function(){
		$(".saveRacePlacement,.updateRacePlacement,.deleteRacePlacement,.publishRacePlacement").click(function(e){
		e.preventDefault();
		var current=$(this);
		...
		$.post("ajax.php?ajaxcall=true&mod=setRacePlacement&action="+action, jsonData, function(response) {
			...
			{
				$(current).html("Published").removeClass("publishRacePlacement");
			}	
		});
	});
});
