...
<div class="row-fluid sortable">
  <div class="box span12">
    <div class="box-header well" data-original-title>
      <h2><i class="icon-user"></i> All Athletic Type </h2>
      <div class="box-icon"></div>
    </div>
    <div class="box-content">
		
  <div class="page-header" style="padding-bottom:5px;margin:8px 0;"><h3>Search</h3></div>
    <form name="frm_search" id="frm_search" method="post" action="index.php?model=athletictype">
    	<input type="hidden" name="action" id="action" value="" />
		<table  style="border:none;" cellpadding="4" cellspacing="2">
            <tr>
                <td align="right">Athletic Type : <input type="search" name="sr_athletic_type_title" id="sr_athletic_type_title" value="<?=$_REQUEST["sr_athletic_type_title"]?>"  /></td>
                <td align="right"></td>
      ...
      <div class="control-group" style="text-align:right;margin:20px 0 0 0;">
        <div class="controls"> 
           <a class="btn btn-success" href="index.php?model=athletictype&action=add"><i class="icon-plus icon-white"></i>New Athletic Type </a> 
        </div>
      </div>
      <br />
      <table class="table table-bordered table-striped table-condensed datatable1">
        <thead>
          <tr>
            <th>Athletic Type #</th>
            <th>Athletic Type Title</th>
			<th>Athletic Type Description</th>
            <th>Add On</th>       
            <th nowrap>Action</th>            
          </tr>
        </thead>
        ...
        <?php 
			...
			{
				$view   = "index.php?model=athletictype&action=view&id=".$datum->id;
				$edit   = "index.php?model=athletictype&action=edit&id=".$datum->id;
			?>
				<tr id="<?=$datum->id?>">
				<td class="center"><?=$datum->id?></td>
				<td class="center"><?=$datum->athletic_type_title?></td>
				<td class="center"><?=$datum->athletic_type_desc?></td>
				<td class="center"><?=date("Y-m-d",strtotime($datum->athletic_type_date))?></td>
				...
				<a class="btn btn-danger delete_datum" id="deleted_<?=$datum->id?>" data="athletic_type;id;<?=$datum->id?>" href="#" alt="Delete Athletes" title="Delete Athletes"> <i class="icon-trash icon-white"></i></a></td>
				</td>
				</tr>
		<?php } ?>
        </tbody>
